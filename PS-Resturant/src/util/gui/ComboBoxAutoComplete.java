...
public class ComboBoxAutoComplete<T> {
    ...
    public void setItems(ObservableList<T> items) {
        ...
        filter = "";
        if (!originalItems.isEmpty() && originalItems.get(0) instanceof Product) {
            cmb.setOnKeyPressed(this::spacificActionForProducts);
        }
    }

    public void spacificActionForProducts(KeyEvent e) {
        ...
        if (!code.equals(KeyCode.TAB) && !code.equals(KeyCode.SHIFT)) {
            ...
            if (filter.length() == 0) {
                ...
                items.filter(product -> unaccent(((Product) product).getName()).toLowerCase().contains(txtUsr)).forEach(filteredList::add);
